...
export default async function handler(
    ...
    /** GPT has limited access tokens, so get the needed get matched sections for each types. If used gpt model has many tokens, we can increase count. Now 1 count = <150 tokens */
    const types = [
        {name: 'fb', count: 5},
        ...
        try {
            if(types[k].name == 'fb') {
                const matched = await supabaseAdmin.rpc("fb_matched_sections", {
                    ...
                    match_threshold: 0.67, /** matched percentage */
                ...
                const matched = await supabaseAdmin.rpc("matched_sections", {
                    ...
                    match_count: types[k].count,
                    trained_type: types[k].name,
                    files_brand_id: brand_id.toString()
                });
                matched_websites = matched.data;
                console.log(matched_websites);

            }
        } catch (error) {
            console.log("--Semantic search error--")
            console.log(error);
        }
    }
    ...
    if(matched_cnt == 0) {
        const empty_res = ["I'm not sure", "I'm not sure", "I'm not sure"];
        full_prompt = `Pleae answer only ${JSON.stringify(empty_res)}`;
    }
    ...
    console.log(data);
    data.choices[0].text = full_prompt;
...
export const config = { /** For streaming in Next JS */
    runtime: "edge",
}